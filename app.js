(() => {
  ...
  passStyle.textContent = `
    ...
    tr.type-change-row td{font-weight:700;font-size:1.05em;padding:3px 8px;border-top:1px solid #222;border-bottom:1px solid #222;text-align:left}
    tr.type-change-row td.type-change-fill{letter-spacing:.01em}
    #detailCrewName.detail-crew-holiday{color:#d60000!important}
    .detail-type-m-wrap{display:flex!important;align-items:stretch;width:100%;height:100%;box-sizing:border-box}
    .detail-type-m-badge{flex:0 0 84px;background:#fff;color:#111;display:flex;align-items:center;justify-content:center;font-size:clamp(38px,5.5vw,68px);font-weight:500;border-right:1px solid #555;box-sizing:border-box}
    .detail-type-m-wrap > #detailType{flex:1 1 auto;min-width:0}
    tr.detail-pass-row td:nth-child(2){font-size:.72em!important;font-weight:400!important}
  `;
  ...
  function normalizeAllTrains() {
    trains = trains.map(train => {
      ...
      return {
        ...
        typeChanges: normalizeTypeChanges(train.typeChanges || train.type_changes || [], stops),
        stops
      };
    });
  }

  function normalizeTypeChanges(changes, stops) {
    if (!Array.isArray(changes)) return [];
    const out = [];
    for (const raw of changes) {
      if (!raw) continue;
      const station = normalizeStationName(raw.station || raw.at || raw.fromStation || "");
      const type = String(raw.type || raw.newType || raw.to || "").trim();
      if (!station || !TRAIN_TYPES.includes(type)) continue;
      const index = stops.findIndex(s => normalizeStationName(s.station) === station);
      if (index < 0) continue;
      if (!out.some(x => x.station === station && x.type === type)) out.push({station, type, index});
    }
    out.sort((a,b) => a.index - b.index);
    return out;
  }

  function effectiveTrainType(train, simulation = null) {
    if (!train) return "";
    let type = train.type || "";
    const changes = Array.isArray(train.typeChanges) ? train.typeChanges : [];
    const sim = simulation || getSimulation();
    const position = sim && Number.isFinite(sim.index) ? sim.index + (Number(sim.progress) || 0) : -1;
    for (const ch of changes) if (position >= ch.index) type = ch.type;
    return type;
  }

  function appendTypeChangeRow(tbody, train, stopIndex, columnCount) {
    if (!tbody || !train) return;
    for (const ch of (train.typeChanges || []).filter(x => x.index === stopIndex)) {
      const tr = document.createElement("tr"); tr.className = "type-change-row";
      const td = document.createElement("td"); td.colSpan = columnCount; td.className = "type-change-fill";
      const style = getTrainTypeStyle(ch.type);
      td.textContent = `${displayStationName(ch.station)}から${style.label || ch.type}`;
      td.style.setProperty("background-color", style.background || "#fff", "important");
      td.style.setProperty("color", style.color || "#111", "important");
      tr.appendChild(td); tbody.appendChild(tr);
    }
  }

  function shortCrewName(crew) {
    const raw = String(crew || "").trim();
    const map = {"名古屋乗務区":"名乗","神宮前乗務区":"神乗","犬山乗務区":"犬乗","知立乗務区":"知乗","瀬戸運輸区":"瀬乗","瀬戸乗務区":"瀬乗"};
    return map[raw] || raw;
  }

  function renderCompactCrewName(train) {
    if (!el.detailCrewName || !train) return;
    const crew = shortCrewName(train.crew || crewForRoute(train.route));
    const holiday = train.dayType === "holiday";
    el.detailCrewName.textContent = holiday ? `${crew}(土休日)` : crew;
    el.detailCrewName.classList.toggle("detail-crew-holiday", holiday);
  }

  // Mは独立した旧セルではなく、種別セルと一体のラッパー内に置く。
  // HTMLのgrid構造に依存しないため、iPadでも位置が変わらない。
  function placeMNextToTrainType() {
    if (!el.detailType || !el.detailCode) return;
    el.detailCode.style.setProperty("display", "none", "important");
    let wrap = document.getElementById("detailTypeMWrap");
    if (!wrap) {
      const type = el.detailType;
      const parent = type.parentElement;
      if (!parent) return;
      wrap = document.createElement("div"); wrap.id = "detailTypeMWrap"; wrap.className = "detail-type-m-wrap";
      parent.insertBefore(wrap, type);
      const badge = document.createElement("div"); badge.className = "detail-type-m-badge"; badge.textContent = "M";
      wrap.appendChild(badge); wrap.appendChild(type);
    }
  }

  // 秒が公開データに無い場合の表示用推定値。公式の「分」は変更しない。
  function ensureEstimatedSeconds(train) {
    if (!train?.stops?.length) return;
    let seed = [...String(train.trainNumber || "")].reduce((a,c)=>a+c.charCodeAt(0),0);
    train.stops.forEach((stop, i) => {
      if (Number.isFinite(Number(stop.seconds))) return;
      const explicit = String(stop.time || "").match(/:(\d{2})$/);
      if (explicit && String(stop.time).split(":").length >= 3) return;
      // 5秒刻みの決定的推定。00固定を避け、公式分内に収める。
      const sec = ((seed + i * 17) % 11 + 1) * 5;
      stop.seconds = Math.min(55, sec);
      stop.secondsEstimated = true;
    });
  }
  ...
  function renderSchedule() {
    ...
    // 検索直後のメイン時刻表でも現在のシミュレーション状態を取得する。
    // detailSimulation は詳細画面内だけのローカル変数なので、ここで参照すると
    // ReferenceError になり selectTrain() が openDetail() まで到達しない。
    const simulation = getSimulation();
    ...
    selectedTrain.stops.forEach(
      (stop, i) => {
        ...
        appendTypeChangeRow(el.scheduleTable, selectedTrain, i, 5);
      }
    );
  }
  ...
  function renderDetail() {
    ...
    ensureEstimatedSeconds(t);
    updateRunningLocation();
    renderCompactCrewName(t);
    placeMNextToTrainType();

    // ★ 詳細画面にも同じ種別カラーを適用
    const detailSimulation = getSimulation();
    applyTrainTypeStyle(
      el.detailType,
      effectiveTrainType(t, detailSimulation)
    );
    ...
    t.stops.forEach((stop, i) => {
      ...
      if (stop.kind === "pass") tr.classList.add("detail-pass-row");
      ...
      appendTypeChangeRow(el.detailSchedule, t, i, 4);
    });
  }
