(() => {
  ...
  const DATA_CACHE_DB = "meitetsu-timetable-cache";
  const DATA_CACHE_STORE = "datasets";
  const DATA_CACHE_KEY = "timetables";
  ...
  function openDataCache() {
    return new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = () => {
        if (!req.result.objectStoreNames.contains(DATA_CACHE_STORE)) {
          req.result.createObjectStore(DATA_CACHE_STORE);
        }
      };
  ...
  async function writeCachedData(data) {
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        tx.objectStore(DATA_CACHE_STORE).put({ savedAt: Date.now(), data });
  ...
  function applyLoadedJson(json) {
    if (Array.isArray(json)) trains = json;
    else if (Array.isArray(json?.trains)) trains = json.trains;
    else trains = [];
    normalizeAllTrains();
    if (dayTypeAuto) activeDayType = getAutoDayType();
    updateDayTypeStatus();
    const dayTrains = getDayFilteredTrains();
    setStatus(`時刻表データ読込完了　全${trains.length.toLocaleString()}列車 / ${getDayTypeLabel(activeDayType)} ${dayTrains.length.toLocaleString()}列車`);
    const real299 = findTrain("299");
    if (real299) {
      selectTrain(real299, false);
      showSearchResult("299", "公式時刻表データから299を取得しました。");
    } else {
      selectTrain(DEMO_299, false);
      showSearchResult("299", "現在のJSONに299がないため、動作確認用データを表示しています。");
    }
    populateLines();
    renderNextTrains();
  }
  ...
  async function loadData() {
    ...
    // まず画面を即時起動。巨大JSONの取得中でも「固まった」ように見せません。
    selectTrain(DEMO_299, false);
    setStatus("時刻表データをバックグラウンドで確認中…");

    try {
      const cached = await readCachedData();
      if (cached?.data) {
        applyLoadedJson(cached.data);
        setStatus(`時刻表データをキャッシュから表示　全${trains.length.toLocaleString()}列車　更新確認中…`);
      }
      ...
      let response;
      try {
        response = await fetch(DATA_URL, { cache: "default", signal: controller.signal });
      } finally {
        clearTimeout(timeout);
      }
      if (!response.ok) throw new Error("HTTP " + response.status);

      setStatus("時刻表データを受信しました。画面を更新中…");
      const json = await response.json();
      await writeCachedData(json);
      applyLoadedJson(json);
    } catch (error) {
      console.error(error);
      if (!trains.length) {
        setStatus("公式時刻表を取得できないため、デモ列車で起動しています。");
        selectTrain(DEMO_299, false);
        showSearchResult("299", "通信またはデータ解析に失敗しました。後で再読み込みすると再取得します。");
      } else {
        setStatus("通信できないため、前回保存した時刻表データを表示しています。");
      }
    } finally {
      loadingData = false;
    }
  ...
  /* 旧loadDataの本体は上で置き換え済み */
  async function loadData_legacy_unused() {
    return;
    if (loadingData) return;
