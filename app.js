(() => {
  ...
  const APP_DATA_COMPAT_VERSION = 12;
  ...
  function renderRoute() {
    ...
    stops.forEach((stop, i) => {
      ...
      const distanceHtml = distance != null
        ? `<div class="station-distance">↓ ${distance.toFixed(1)} km</div>`
        : "";
    ...
    selectedTrain.stops.forEach(
      (stop, i) => {
        ...
        tr.innerHTML = `
          ...
          <td>${stop.kind === "pass" ? "—" : escapeHtml(stop.time || "—")}</td>
          <td>${distance != null ? `${distance.toFixed(1)} km / ` : ""}${escapeHtml(status)}</td>
  ...
  function renderDetail() {
    ...
    t.stops.forEach((stop, i) => {
      ...
      tr.innerHTML = `
        <td>${i === simulation.index ? "●" : ""}</td>
        <td>${escapeHtml(displayStationName(stop.station))}${stop.kind === "pass" ? ' <small class="pass-label">通過</small>' : ''}${distance != null ? `<small class="pass-label">${distance.toFixed(1)} km</small>` : ''}</td>
