(() => {
  ...
  function renderDetail() {
    ...
    if (el.detailCars) {
      el.detailCars.textContent =
        Number.isFinite(Number(t.cars)) && Number(t.cars) > 0 ? Number(t.cars) : "0";
    }
  ...
  // 両数は「両」の数字部分を直接タップして入力します。
  // PDFの両数は使用せず、入力値だけを selectedTrain.cars に反映します。
  function initDirectCarsInput() {
    ...
    target.addEventListener("click", (ev) => {
      // 1回タップで現在の数字を選択し、そのまま入力できるようにする。
      ev.stopPropagation();
      selectAll();
    });

    target.addEventListener("keydown", (ev) => {
      if (["Backspace", "Delete", "ArrowLeft", "ArrowRight", "Home", "End", "Tab"].includes(ev.key)) return;
      if ((ev.ctrlKey || ev.metaKey) && ["a", "c", "v", "x"].includes(ev.key.toLowerCase())) return;
    ...
    target.addEventListener("input", () => {
      ...
      let raw = String(target.textContent || "").replace(/[^0-9]/g, "");
      ...
      selectedTrain.cars = Number.isFinite(n) && n > 0 ? Math.floor(n) : null;

      // 上部の列車表示と停車警告にも即時反映。
      if (el.cars) {
        el.cars.textContent = selectedTrain.cars ? `${selectedTrain.cars}両` : "—";
      }
      updateDriverStopAlert();
    });

    target.addEventListener("blur", () => {
      // 空欄で終了した場合も「0両」として表示を維持。
      const raw = String(target.textContent || "").replace(/[^0-9]/g, "");
      target.textContent = raw || "0";
    });
  }
