(() => {
  ...
  function appendTypeChangeRow(tbody, train, stopIndex, columnCount) {
    ...
    const changes = (train.typeChanges || []).filter(ch => ch.index === stopIndex);
    for (const ch of changes) {
      const tr = document.createElement("tr");
      tr.className = "type-change-row";
      const td = document.createElement("td");
      td.colSpan = columnCount;
      td.className = "type-change-fill";
      const style = getTrainTypeStyle(ch.type);
      td.textContent = `${displayStationName(ch.station)}から${style.label || ch.type}`;
      td.style.setProperty("background-color", style.background || "#ffffff", "important");
      td.style.setProperty("color", style.color || "#111111", "important");
      tr.appendChild(td);
