(() => {
  ...
  // 停車確認済みの駅。同じ停車イベント中は再表示しない。
  let confirmedAlertStopIndex = -1;
  ...
  function renderDetail() {
    ...
    if (el.detailCars) {
      el.detailCars.textContent =
        Number.isFinite(Number(t.cars)) ? Number(t.cars) : "0";
    }
  ...
  function updateDriverStopAlert() {
    ...
    selectedTrain.stops.forEach((stop, index) => {
      ...
      if (inWindow || dwell) {
        if (!candidate || delta < candidate.delta) candidate = { stop, index, delta };
      }
    });
    ...
    // 「停車確認」を押した停車駅については、次の停車駅へ進むまで再表示しない。
    if (confirmedAlertStopIndex === candidate.index) {
      box.hidden = true;
      return;
    }
    ...
    if (alertStopIndex !== candidate.index) {
      ...
      if (button) { button.textContent = "停車確認"; button.classList.remove("confirmed"); }
    }
    const times = getAlertTimes(stop);
    document.getElementById("alertCars").textContent =
      Number.isFinite(Number(stop.cars)) ? Number(stop.cars) : (Number.isFinite(Number(selectedTrain.cars)) ? Number(selectedTrain.cars) : 0);
    document.getElementById("alertStation").textContent = `${displayStationName(stop.station)}停車`;
  ...
  function initStopSimulator() {
    // 赤パネルを画面全体ではなく、左側の運転台表示領域へ移動。
    const stopAlert = document.getElementById("stopAlert");
    const detailLeft = document.querySelector(".detail-left");
    if (stopAlert && detailLeft && stopAlert.parentElement !== detailLeft) {
      detailLeft.appendChild(stopAlert);
    }
    ...
    document.getElementById("stopConfirmButton")?.addEventListener("click", () => {
      const b = document.getElementById("stopConfirmButton");
      const box = document.getElementById("stopAlert");
      // 押した瞬間に消し、同じ停車駅では再表示しない。
      confirmedAlertStopIndex = alertStopIndex;
      if (box) box.hidden = true;
      if (b) { b.textContent = "停車確認"; b.classList.remove("confirmed"); }
    });
