(() => {
  ...
  const el = {
    ...
    detailCarsInput: $("detailCarsInput"),
  ...
  function renderDetail() {
    ...
    if (el.detailCarsInput) {
      el.detailCarsInput.value =
        Number.isFinite(Number(t.cars)) && Number(t.cars) > 0 ? String(Number(t.cars)) : "";
    }
  ...
  function updateDriverStopAlert() {
    ...
    const cars = Number.isFinite(Number(stop.cars))
      ? Number(stop.cars)
      : (Number.isFinite(Number(selectedTrain.cars)) ? Number(selectedTrain.cars) : 0);

    document.getElementById("alertCars").textContent = String(cars);
  ...
  function initManualCarsInput() {
    const input = document.getElementById("detailCarsInput");
    if (!input) return;
    input.addEventListener("input", () => {
      ...
      selectedTrain.cars = Number.isFinite(n) && n > 0 ? Math.min(20, Math.floor(n)) : null;
      renderTrain();
      updateDriverStopAlert();
    });
  }
