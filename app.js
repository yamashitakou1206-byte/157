(() => {
  ...
  const APP_DATA_COMPAT_VERSION = 10;
  ...
  function displayStationName(value) {
    const raw = normalizeStationName(value);
    return STATION_DISPLAY_NAME[raw] || raw;
  }
  ...
  function normalizeStops(train) {
    if (Array.isArray(train.stops)) {
      ...
      for (const x of train.stops) {
        ...
        result.push({ station, time, pass: Boolean(x.pass) });
      }
    ...
    if (
      ...
      train.stations.forEach((station, i) => {
        ...
        result.push({ station: name, time, pass: false });
      });
  ...
  function findTrain(number) {
    ...
    // 異なる運転日のデータも列車番号検索では候補に残す。
    // 日種別フィルターで9,000件前後に見えても、全列車データを失わない。
    const anyDay = trains.filter(train =>
      String(train.trainNumber).trim().toUpperCase() === target
    );
    if (anyDay.length) return chooseTrainCandidate(anyDay);
  ...
  function renderTrain() {
    ...
    if (el.cars) {
      el.cars.textContent =
        (carsForType(t.type, t) ? carsForType(t.type, t) + "両" : "—");
    }
  ...
  function carsForType(type, train = selectedTrain) {
    const n = Number(train?.cars ?? train?.carCount);
    return Number.isFinite(n) && n > 0 ? n : null;
  }

  // =========================================================
  // 通過時刻（秒）・停車/通過判定
  // 公式時刻表は分単位なので、秒は「シミュレーション推定値」。
  // 通過駅は前後の公式時刻の間を駅数で補間する。
  // =========================================================
  function timeToSeconds(time) {
    const [h,m] = String(time || "").split(":").map(Number);
    return Number.isFinite(h) && Number.isFinite(m) ? h*3600+m*60 : NaN;
  }

  function enrichStopTimes(stops) {
    const out = stops.map((s,i) => ({...s, estimatedSeconds: NaN, estimated: false, pass: Boolean(s.pass)}));
    for (let i=0;i<out.length;i++) {
      if (out[i].time) { out[i].estimatedSeconds=timeToSeconds(out[i].time); continue; }
      if (!out[i].pass) continue;
      let a=i-1; while(a>=0 && !out[a].time) a--;
      let b=i+1; while(b<out.length && !out[b].time) b++;
      if (a>=0 && b<out.length) {
        let sa=timeToSeconds(out[a].time), sb=timeToSeconds(out[b].time);
        if (sb < sa) sb += 86400;
        const span=b-a;
        out[i].estimatedSeconds=sa+(sb-sa)*(i-a)/span;
  ...
  function getDisplayStopTime(stop) {
    if (stop.estimatedSeconds != null && Number.isFinite(stop.estimatedSeconds)) return formatSeconds(stop.estimatedSeconds);
    return stop.time || "--:--";
  }
  ...
  function getSimulation(now = new Date()) {
    ...
    const times = stops.map(s => {
      if (s.estimatedSeconds != null && Number.isFinite(s.estimatedSeconds)) return s.estimatedSeconds / 60;
    ...
    selectedTrain.stops.forEach(
      (stop, i) => {
        ...
        if (i < simulation.index) {
          ...
          status = stop.pass ? "通過" : (simulation.state === "停車" ? "停車中" : "現在位置");
        } else if (
        ...
        tr.innerHTML = `
          <td>${escapeHtml(displayStationName(stop.station))}</td>
          <td>${escapeHtml(stop.time || getDisplayStopTime(stop))}</td>
          <td>${escapeHtml(getDisplayStopTime(stop))}${stop.estimated ? "（推定）" : ""}</td>
  ...
  function updateCountdown() {
    ...
    const nextStop = selectedTrain.stops[nextIndex];
    const target = Number.isFinite(nextStop.estimatedSeconds)
      ? nextStop.estimatedSeconds / 60
      : timeToMinutes(nextStop.time);
