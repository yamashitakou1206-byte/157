(() => {
  ...
  // =========================================================
  // 動作確認用299
  // 実データに299が存在すれば自動的に実データへ切り替わります
  // =========================================================
  const DEMO_299 = {
    trainNumber: "299",
    type: "快速特急",
  ...
  function normalizeStops(train) {
    ...
    if (
      ...
      return train.stations.map((station, i) => ({
  ...
  function getSimulation(now = new Date()) {
    ...
    if (currentMinutes >= times[times.length - 1]) {
      return {
    ...
    for (let i = 0; i < times.length - 1; i++) {
      ...
      if (currentMinutes >= a && currentMinutes < b) {
  ...
  function renderRoute() {
    ...
    line.className = "sim-route-line";

    stops.forEach((stop, i) => {

      const node = document.createElement("div");

      node.className = "sim-station";
  ...
  function renderSchedule() {
    if (!el.scheduleTable || !selectedTrain) {
      return;
    }
    ...
    selectedTrain.stops.forEach((stop, i) => {
      ...
      if (el.detailCrewState) {
        if (simulation.state === "走行中") {
          el.detailCrewState.textContent =
            "運転中";
        } else if (simulation.state === "停車") {
          el.detailCrewState.textContent =
            "停車中";
        } else if (simulation.state === "終着") {
          el.detailCrewState.textContent =
            "終着";
        } else {
          el.detailCrewState.textContent =
