(() => {
  ...
  function railTimeParts(value, extraSeconds = null) {
    ...
    if (!m) return { main: raw || "—", seconds: extraSeconds != null ? String(extraSeconds).padStart(2, "0") : "" };
    const sec = m[3] != null ? m[3] : (extraSeconds != null && Number.isFinite(Number(extraSeconds)) ? String(Number(extraSeconds)).padStart(2, "0") : "");
  ...
  function railTimeHtml(value, extraSeconds = null) {
    ...
    const cls = p.seconds === "00" ? "time-sec time-sec-zero" : "time-sec";
    return `${escapeHtml(p.main)}<span class="${cls}">${escapeHtml(p.seconds)}</span>`;
  }
