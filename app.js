...
function isOfficialReady(){return !!(officialData&&Array.isArray(officialData.trains)&&officialData.trains.length)}

async function loadOfficialData(){
  if(officialDataPromise)return officialDataPromise;
  officialDataPromise=fetch('./data/timetables.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('data');return r.json()}).then(d=>{
    officialData=d;
    const count=d.trains?.length||0;
    const el=$('dataStatus');
    if(el){el.className=count?'data-status ok':'data-status warn';el.textContent=count?`公式PDF解析データ：${count}列車・${d.timetableRevision||'—'}改正`:'公式PDF解析データはまだ生成されていません。Actionsから更新してください。'}
    return d;
...
function findOfficialTrains(q){
  ...
  const list=officialData.trains.filter(t=>normalizeTrainNo(t.no)===n);
  const wanted=holiday()?'holiday':'weekday';
  return list.filter(t=>!t.day||t.day===wanted).concat(list.filter(t=>t.day&&t.day!==wanted));
}
...
function detailFromOfficial(t){
  const stops=(t.stops||[]).filter(x=>x&&x[0]&&x[1]).map(x=>[x[0],x[1],x[1]]);
  return {type:typeFull(t.type),code:t.route?.match(/名古屋本線/)?'M':'—',cars:'—',crew:crewForRoute(t.route),origin:stops[0]?.[0]||'—',destination:t.destination||stops.at(-1)?.[0]||'—',notice:`名鉄公式路線別時刻表 / ${t.day==='holiday'?'土・休日':'平日'} / ${t.route||'—'}`,state:'時刻表',stops,no:t.no,route:t.route,day:t.day,pdf:t.pdf};
}
...
function searchTrain(){
  ...
  if(found.length){
    $('trainSearchResult').innerHTML=found.slice(0,6).map((t,i)=>`<button class="search-train-result" data-i="${i}"><b>${t.no}</b><span>${typeFull(t.type)}　${t.destination||'—'}</span><small>${t.route||'—'} / ${t.day==='holiday'?'土・休日':'平日'} / ${t.direction==='down'?'下り':'上り'}</small></button>`).join('');
...
load();state.favorites=Array.isArray(state.favorites)?state.favorites:[];initSelectors();document.documentElement.style.setProperty('--zoom',state.zoom||1);bind();update();loadOfficialData();
