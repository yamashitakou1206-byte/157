(() => {
  ...
  function normalizeStationName(value) {
    ...
    if (bad.has(s)) return "";
    // PDF見出しが駅名へ付着した代表例（土青山/平青山）。
    // パーサー側で直すのが本体だが、旧JSONを読んだ時の表示安全網。
    const weekdayPrefix = s.match(/^[土平](青山)$/);
    if (weekdayPrefix) return weekdayPrefix[1];
  ...
  function normalizeStops(train) {
    if (Array.isArray(train.stops)) {
      ...
      for (const x of train.stops) {
        ...
        else { stop.seconds = 0; stop.secondsEstimated = true; }
