...
def resolve_parser():
    """v12パーサーを安全に探す。標準名を最優先し、名前違いも診断する。"""
    if PARSER.is_file():
        return PARSER

    candidates = []
    if TOOLS_DIR.exists():
        for p in sorted(TOOLS_DIR.glob("build_timetable_fixed_v12*.py")):
            if p.is_file():
                candidates.append(p)

    print()
    print("=" * 72)
    print("Parser file diagnostics")
    print("=" * 72)
    print(f"Expected : {PARSER}")
    ...
    if len(candidates) == 1:
        found = candidates[0]
        print()
        print("WARNING: 標準名ではありませんが、v12候補を1件検出しました。")
        print(f"Using    : {found}")
        print("GitHub上では build_timetable_fixed_v12.py への変更を推奨します。")
        return found

    if len(candidates) > 1:
        names = "\n".join(f"  - {p.name}" for p in candidates)
        raise FileNotFoundError(
            "標準名のv12パーサーがなく、候補が複数あるため自動選択できません。\n"
            f"必要な名前: {PARSER.name}\n"
            f"候補:\n{names}"
        )
    ...
    raise FileNotFoundError(
        "v12パーサーが見つかりません。\n"
        f"必要な場所: {PARSER}\n"
        "tools フォルダへ build_timetable_fixed_v12.py をコミットしてください。"
    )
...
def run_parser():
    ...
    print()
    print("=" * 72)
    print("Running timetable parser v12")
    print("=" * 72)
    print(f"Parser file: {parser}")
    print(f"Parser size: {parser.stat().st_size:,} bytes")
    ...
    try:
        ...
        raise RuntimeError(
            f"v12パーサーが終了コード {e.returncode} で失敗しました。"
        ) from e

    # 出力確認
    if not OUTPUT.exists():
        raise FileNotFoundError(
            "v12の実行後も data/timetables.json が生成されませんでした"
        )
