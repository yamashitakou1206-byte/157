...
# -----------------------------------------------------------------------------
# merge-16: coordinate-primary parser
# -----------------------------------------------------------------------------
# Official timetable PDFs are laid out as a fixed grid. pdfplumber's table
# extraction can merge many vertical station cells into one cell, which caused
# fragments such as "名" / "河" and could shift a time into the wrong station.
# This parser instead reads the visible glyph geometry: train columns are taken
# from the header row, station names are reconstructed from all glyphs in the
# station-name band, and times/pass marks are read at the matching row/column.

_JP_TOKEN_RE = re.compile(r"[一-龥々ヶケぁ-んァ-ヶー]")
_COORD_BANNED_STATIONS = {
    "列車番号", "種別", "行先", "行き先", "記事", "次のページ", "前のページ",
    "始発", "終着", "平日", "土休日", "休日", "下り", "上り", "改正",
}


def _visible_words(page) -> list[dict[str, Any]]:
    try:
        words = page.extract_words(
            x_tolerance=1.2,
            y_tolerance=1.8,
    ...
    for w in words:
        try:
            x0, x1 = float(w.get("x0", 0)), float(w.get("x1", 0))
            top, bottom = float(w.get("top", 0)), float(w.get("bottom", 0))
        except Exception:
            continue
        if x1 < 0 or x0 > float(page.width) or bottom < 0 or top > float(page.height):
            continue
        visible.append(w)
    return visible


def _cluster_coord_lines(words: list[dict[str, Any]], tolerance: float = 1.6) -> list[dict[str, Any]]:
    lines: list[dict[str, Any]] = []
    for w in sorted(words, key=lambda z: (_word_cy(z), float(z.get("x0", 0)))):
        cy = _word_cy(w)
        target = None
        for line in reversed(lines[-8:]):
            if abs(float(line["cy"]) - cy) <= tolerance:
                target = line
                break
        if target is None:
            target = {"cy": cy, "words": []}
        ...
        target["cy"] = sum(_word_cy(x) for x in target["words"]) / len(target["words"])
    for line in lines:
        line["words"].sort(key=lambda z: float(z.get("x0", 0)))
    return lines


def _coord_japanese(value: Any) -> str:
    s = clean_station(value)
    return re.sub(r"[^一-龥々ヶケぁ-んァ-ヶー]", "", s)


def _coord_header(lines: list[dict[str, Any]]) -> Optional[tuple[int, list[tuple[dict[str, Any], str]]]]:
    """Find the actual train-number header, not 3/4 digit timetable rows."""
    best: Optional[tuple[tuple[int, int, float], int, list[tuple[dict[str, Any], str]]]] = None
    for i, line in enumerate(lines):
        if float(line["cy"]) > 130:
            continue
        nums: list[tuple[dict[str, Any], str]] = []
        for w in line["words"]:
            n = normalize_train_number(w.get("text", ""))
            if n and TRAIN_RE.fullmatch(n):
                nums.append((w, n))
        if not nums:
            continue

        # A real header has service-type text directly below the same columns.
        aligned_types = 0
        for w, _ in nums:
            x = _word_cx(w)
            found = False
            for lower in lines:
                if not (float(line["cy"]) + 4 <= float(lower["cy"]) <= float(line["cy"]) + 32):
                    continue
                for tw in lower["words"]:
                    if abs(_word_cx(tw) - x) <= 10 and normalize_type(tw.get("text", "")):
                        found = True
                        break
                if found:
                    break
        ...
        score = (aligned_types, len(nums), -float(line["cy"]))
...
def _coord_column_bounds(centers: list[float]) -> list[tuple[float, float]]:
    out: list[tuple[float, float]] = []
    for i, x in enumerate(centers):
        if len(centers) == 1:
            out.append((x - 12, x + 12))
            continue
        left = (centers[i - 1] + x) / 2 if i else x - (centers[1] - x) / 2
        right = (x + centers[i + 1]) / 2 if i < len(centers) - 1 else x + (x - centers[i - 1]) / 2
        out.append((left, right))
    return out


def _coord_cell_type(line: Optional[dict[str, Any]], lo: float, hi: float) -> str:
    if not line:
        return ""
    for w in line["words"]:
        if lo - 1 <= _word_cx(w) <= hi + 1:
            typ = normalize_type(w.get("text", ""))
            if typ:
                return typ
    return ""


def _coord_cell_destination(lines: list[dict[str, Any]], lo: float, hi: float) -> str:
    parts: list[str] = []
    for line in lines:
        row: list[str] = []
        for w in line["words"]:
            if lo - 1 <= _word_cx(w) <= hi + 1:
                s = _coord_japanese(w.get("text", ""))
                if s:
                    row.append(s)
        if row:
            parts.append("".join(row))
    return clean_station("".join(parts))


def _coord_event_cell(line: dict[str, Any], x: float) -> tuple[Optional[str], bool]:
    candidates: list[tuple[float, Optional[str], bool]] = []
    for w in line["words"]:
        if abs(_word_cx(w) - x) > 7.0:
            continue
        raw = w.get("text", "")
        t = normalize_time(raw)
        p = is_pass(raw)
        if t or p:
            candidates.append((abs(_word_cx(w) - x), t, p))
    if not candidates:
        return None, False
    candidates.sort(key=lambda x: x[0])
...
def parse_page_coordinates(page, *, pdf_name: str, page_number: int) -> list[dict[str, Any]]:
    words = _visible_words(page)
    if not words:
        return []
    lines = _cluster_coord_lines(words)
    header = _coord_header(lines)
    if not header:
        return []

    header_i, header_nums = header
    header_y = float(lines[header_i]["cy"])
    cols = sorted([(_word_cx(w), n) for w, n in header_nums], key=lambda z: z[0])
    if not cols:
        return []
    centers = [x for x, _ in cols]
    bounds = _coord_column_bounds(centers)
    train_x_lo = centers[0] - 10
    train_x_hi = centers[-1] + 10

    # Type row: highest number of valid service types below the train-number header.
    type_candidates: list[tuple[int, dict[str, Any]]] = []
    for line in lines:
        cy = float(line["cy"])
        if not (header_y + 4 <= cy <= header_y + 32):
            continue
        count = sum(1 for w in line["words"] if normalize_type(w.get("text", "")))
        if count:
            type_candidates.append((count, line))
    type_line = max(type_candidates, key=lambda x: x[0])[1] if type_candidates else None
    type_y = float(type_line["cy"]) if type_line else header_y + 15

    # Destination can wrap over two narrow rows (notably 築港線). Select the
    # contiguous Japanese rows with broad coverage across train columns.
    dest_candidates: list[dict[str, Any]] = []
    coverage_min = max(2, min(5, len(cols) // 4))
    for line in lines:
        cy = float(line["cy"])
        if not (type_y + 7 <= cy <= header_y + 52):
            continue
        count = sum(
            1 for w in line["words"]
            if train_x_lo <= _word_cx(w) <= train_x_hi
            and _JP_TOKEN_RE.search(_coord_japanese(w.get("text", "")))
            and not normalize_type(w.get("text", ""))
        )
        if count >= coverage_min:
            dest_candidates.append(line)

    dest_lines: list[dict[str, Any]] = []
    if dest_candidates:
        groups: list[list[dict[str, Any]]] = []
        group: list[dict[str, Any]] = []
        for line in sorted(dest_candidates, key=lambda x: float(x["cy"])):
            if not group or float(line["cy"]) - float(group[-1]["cy"]) <= 8:
                group.append(line)
            else:
                groups.append(group)
                group = [line]
        ...
        def group_score(g: list[dict[str, Any]]) -> tuple[int, float]:
            total = 0
            for line in g:
                total += sum(
                    1 for w in line["words"]
            ...
            return total, -float(g[0]["cy"])

        dest_lines = max(groups, key=group_score)

    body_start = (max(float(x["cy"]) for x in dest_lines) + 8) if dest_lines else header_y + 55

    # Rows that actually contain a timetable token in at least one train column.
    data_lines: list[dict[str, Any]] = []
    for line in lines:
        if float(line["cy"]) < body_start:
            continue
        has_data = any(
            train_x_lo <= _word_cx(w) <= train_x_hi
            and (normalize_time(w.get("text", "")) is not None or is_pass(w.get("text", "")))
            for w in line["words"]
        )
        if has_data:
    ...
    # The station-name column alternates between left and right on facing pages.
    # Derive both possible adjacent bands from the train columns, then pick the
    # side yielding the larger set of valid station rows.
    bands = [
        ("left", max(0.0, centers[0] - 75), centers[0] - 12),
        ("right", centers[-1] + 12, min(float(page.width), centers[-1] + 75)),
    ]

    def station_candidates(lo: float, hi: float) -> list[tuple[float, str]]:
        ...
        for line in lines:
            ...
            pieces: list[str] = []
            for w in line["words"]:
                if not (lo <= _word_cx(w) <= hi):
                    continue
                token = _coord_japanese(w.get("text", ""))
                if token in {"発", "着", "〃"}:
                    continue
                if token:
                    pieces.append(token)
            station = clean_station("".join(pieces))
            if not station or station in _COORD_BANNED_STATIONS:
                continue
            # There are no one-character Meitetsu station names in the current
            # route timetable set; rejecting them prevents "名"/"河" fragments.
            if not (2 <= len(station) <= 14):
                continue
            if not _JP_TOKEN_RE.search(station):
                continue
            if not any(abs(float(dl["cy"]) - cy) <= 5.5 for dl in data_lines):
                continue
            found.append((cy, station))
        return found

    side_results = [(side, station_candidates(lo, hi)) for side, lo, hi in bands]
    _, stations = max(side_results, key=lambda x: len(x[1]))
    if not stations:
        return []

    # Stable de-duplication of station rows.
    compact_stations: list[tuple[float, str]] = []
    seen_station_rows: set[tuple[float, str]] = set()
    for cy, station in stations:
        key = (round(cy, 1), station)
        if key in seen_station_rows:
            continue
        seen_station_rows.add(key)
        compact_stations.append((cy, station))
    ...
    for col_i, (x, train_number) in enumerate(cols):
        train_type = _coord_cell_type(type_line, *bounds[col_i])
        if not train_type:
            continue
        destination = _coord_cell_destination(dest_lines, *bounds[col_i])
        if destination in SKIP_LABELS:
            destination = ""

        stops: list[dict[str, Any]] = []
        for station_y, station in compact_stations:
            nearby = sorted(
                [dl for dl in data_lines if abs(float(dl["cy"]) - station_y) <= 5.5],
                key=lambda z: float(z["cy"]),
            )
            values: list[tuple[float, Optional[str], bool]] = []
            for dl in nearby:
                t, passed = _coord_event_cell(dl, x)
                if t or passed:
                    values.append((float(dl["cy"]), t, passed))
            if not values:
                continue

            timed = [(cy, t) for cy, t, passed in values if t]
            if not timed:
                if any(passed for _, _, passed in values):
                    stops.append({"station": station, "time": None, "pass": True})
                continue

            # A station name sits between arrival and departure rows on many
            # pages. If two timed rows are present, retain both explicitly.
            stop: dict[str, Any] = {
                "station": station,
                "time": timed[-1][1],
                "pass": False,
            ...
            if len(timed) >= 2:
                stop["arrival"] = timed[0][1]
                stop["departure"] = timed[-1][1]
            else:
                stop["departure"] = timed[0][1]
            stops.append(stop)

        timed_count = sum(1 for s in stops if s.get("time"))
        if timed_count < 2 or not chronological(stops):
            continue
        ...
        trains.append({
            ...
            "direction": direction,
            "crew": crew_for_route(route),
            "origin": stops[0].get("station", "") if stops else "",
            "stops": stops,
            "_source": {
                "pdf": pdf_name,
                "page": page_number,
...
def canonicalize_destinations(trains: list[dict[str, Any]]) -> int:
    """Expand only unambiguous destination fragments using parsed official station names."""
    master = sorted({
        clean_station(s.get("station", ""))
        for t in trains for s in (t.get("stops") or [])
        if 2 <= len(clean_station(s.get("station", ""))) <= 14
    })
    repaired = 0
    for t in trains:
        dest = clean_station(t.get("destination", ""))
        if not dest or dest in master:
            continue
        if len(dest) < 2:
            continue
        matches = [x for x in master if x.startswith(dest) or x.endswith(dest)]
        if len(matches) == 1:
            t["destination"] = matches[0]
            repaired += 1
    return repaired
...
def parse_pdf(path: Path) -> list[dict[str, Any]]:
    ...
    with pdfplumber.open(path) as pdf:
        for page_no, page in enumerate(pdf.pages, start=1):
            # merge-16: coordinate parser is authoritative for normal timetable
            # pages. Table extraction is now only a fallback for unusual layouts.
            coordinate = parse_page_coordinates(
                page,
                pdf_name=path.name,
                page_number=page_no,
            )
            if coordinate:
                ...
                print(f"  page {page_no:>2}: {len(coordinate):>4} trains [coordinate]")
                continue

            type_fallback = coordinate_type_candidates(page)
            station_lexicon = sorted(
                set(page_station_lexicon(page)) | set(coordinate_station_lexicon(page)),
                key=lambda x: (-len(x), x),
            )
            tables = extract_page_tables(page)
            page_result: list[dict[str, Any]] = []
            for table_no, table in enumerate(tables, start=1):
                page_result.extend(parse_table(
                    table,
                    pdf_name=path.name,
                    page_number=page_no,
            ...
            # Do not publish obvious one-character station fragments even in fallback.
            page_result = [
                t for t in page_result
                if all(2 <= len(clean_station(s.get("station", ""))) <= 14 for s in (t.get("stops") or []))
            ]
            result.extend(page_result)
            print(f"  page {page_no:>2}: {len(page_result):>4} trains [table-fallback]")
...
def validate_train(t: dict[str, Any]) -> bool:
    ...
    if not isinstance(stops, list) or len(stops) < 2:
        return False
    for stop in stops:
        station = clean_station(stop.get("station", ""))
        if not (2 <= len(station) <= 14):
            return False
        if not _JP_TOKEN_RE.search(station):
            return False
...
def main() -> int:
    ...
    print("=" * 72)
    print("Meitetsu timetable parser v12 / merge-16-coordinate-primary")
    ...
    destination_repairs = canonicalize_destinations(all_trains)
    print(f"Destination fragment repairs: {destination_repairs}")
    ...
    # Keep every official route segment for completeness/searchability, then add
    # safely merged through-service candidates. app.js prefers the most complete
    # candidate for the same train number, so a through train can reach its true
    # destination without deleting the original source segments.
    preserved = list(valid_before_merge)
    print(f"Preserved raw timetable segments: {len(preserved)}")
    logical = merge_segments([dict(t) for t in valid_before_merge])
    combined = []
    for t in logical:
        if len(_segment_sources(t)) >= 2:
            t["combined"] = True
            combined.append(t)
    print(f"Combined through-service candidates: {len(combined)}")
    merged = final_dedupe(preserved + combined)
    ...
    if len(merged) < MIN_TRAIN_COUNT:
        raise RuntimeError(
            ...
            f"(minimum={MIN_TRAIN_COUNT}; merge-16 coordinate-primary guard)"
        )

    payload = {
        "version": 12,
        "build": "12-coordinate-primary-merge-16",
        "segmentCount": len(preserved),
        "combinedTrainCount": len(combined),
