...
- 「レ」（通過）を保持
- PDF跨ぎは、同一列車番号・同一接続駅・30分以内の場合だけ安全に結合
...
def normalize_type(value: Any) -> str:
    ...
    # 注記等が同じセルに付いても、既知の種別だけを拾う。
    # 長い語を先に見る。
    for key in ("快速特急", "ミュースカイ", "快速急行", "快特", "特急", "準急", "急行", "普通", "μＳ", "μS", "μs", "快急"):
        if key in s:
...
def parse_table(
    ...
    *,
    ...
    for block_no, header_i in enumerate(train_headers):
        ...
        if type_rel is None:
            # 種別が取れないブロックは「普通」に決め打ちせず捨てる
            continue

        body_start = max(type_rel, dest_rel if dest_rel is not None else type_rel) + 1
...
def extract_page_tables(page) -> list[list[list[Any]]]:
    ...
    for settings in TABLE_SETTINGS:
        ...
        # linesで列車番号表が見つかっていれば、それを優先
        if any(
            any(contains_header([clean(c) for c in (row or [])], HEADER_TRAIN) for row in table)
    ...
    for key, segments in by_number.items():
        ...
        while changed:
            ...
            for i, a in enumerate(segments):
                ...
                # 最後に「時刻」がある駅を接続点にする。
                a_timed = [s for s in a["stops"] if s.get("time")]
                if not a_timed:
                    continue
                a_end = a_timed[-1]

                for j, b in enumerate(segments):
                    if i == j or not b.get("stops"):
                        continue

                    b_timed = [s for s in b["stops"] if s.get("time")]
                    if not b_timed:
                        continue
                    b_start = b_timed[0]

                    if a_end["station"] != b_start["station"]:
                        continue

                    ta = time_minutes(a_end["time"])
                    tb = time_minutes(b_start["time"])
                    gap = tb - ta

                    if gap < 0:
                        # 深夜だけ日跨ぎ候補とする。
                        if ta >= 20 * 60 and tb <= 3 * 60:
                            gap += 1440
                        else:
                            continue
                    ...
                    # 同じPDF・ページ・列由来のもの同士は
                    # 本来1セグメントのため、ここでは結合候補にしない。
                    sa = a.get("_source", {})
                    sb = b.get("_source", {})
                    if (
                        sa.get("pdf") == sb.get("pdf")
                        and sa.get("page") == sb.get("page")
                        and sa.get("table") == sb.get("table")
                    ):
                        continue
            ...
            best = [x for x in candidates if x[0] == best_gap]
            ...
            combined_stops = list(a["stops"])

            # b先頭の接続駅はa末尾と同一駅なので、
            # 時刻が違う場合は同駅の発時刻として統合する。
            b_stops = list(b["stops"])
            if b_stops:
                first = b_stops[0]
                if combined_stops and first.get("station") == combined_stops[-1].get("station"):
                    if first.get("time"):
                        combined_stops[-1]["departure"] = first["time"]
                        combined_stops[-1]["time"] = first["time"]
                    b_stops = b_stops[1:]

            for stop in b_stops:
                if (
                    combined_stops
                    and stop.get("station") == combined_stops[-1].get("station")
                    and stop.get("time") == combined_stops[-1].get("time")
                ):
                    continue
                combined_stops.append(stop)

            if not chronological(combined_stops):
                # この候補は使わず、曖昧な連結を避ける。
                break

            combined = dict(a)
            combined["stops"] = combined_stops
            combined["destination"] = b.get("destination") or a.get("destination", "")

            routes = []
            for route in (a.get("route", ""), b.get("route", "")):
            ...
            # 種別は区間途中で変わる列車がある。
            # UI互換のtypeは先頭区間を維持し、変化はtypeChangesへ保存。
            changes = list(a.get("typeChanges", []))
            if b.get("type") and b.get("type") != a.get("type"):
                changes.append({
                    "station": b["stops"][0].get("station", "") if b.get("stops") else "",
                    "type": b["type"],
                })
            changes.extend(b.get("typeChanges", []))
            if changes:
                combined["typeChanges"] = changes

            sources = []
            for src in (a.get("_sources") or [a.get("_source", {})]):
                if src and src not in sources:
                    sources.append(src)
            for src in (b.get("_sources") or [b.get("_source", {})]):
                if src and src not in sources:
                    sources.append(src)
            ...
            identity = json.dumps(
                {
                    ...
                    "trainNumber": combined["trainNumber"],
            ...
            segments = [
                segment for k, segment in enumerate(segments)
                if k not in (i, j)
            ]
