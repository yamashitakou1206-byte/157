...
def parse_pdf(path: Path) -> list[dict[str, Any]]:
    ...
    with pdfplumber.open(path) as pdf:
        for page_no, page in enumerate(pdf.pages, start=1):
            ...
            station_lexicon = page_station_lexicon(page)
...
def reconcile_calendar_station_names(trains: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """平日/土休日PDF間で駅名を照合し、曜日見出し等が駅名へ混入したケースを補正する。

    例: holiday側の「土青山」に対して weekday側に「青山」が存在する場合だけ
    「青山」へ直す。単独PDFだけを見て「土」を削ることはしないため、実在する
    「土橋」のような駅名を壊さない。
    """
    station_sets: dict[tuple[str, str, str], set[str]] = defaultdict(set)
    for t in trains:
        key = (str(t.get("route", "")), str(t.get("direction", "")), str(t.get("dayType", "")))
        for stop in t.get("stops", []):
            st = clean_station(stop.get("station", ""))
            if 1 < len(st) <= 14:
                station_sets[key].add(st)

    marker_chars = set("平土休日上下往復")
    repaired = 0

    def repair(name: Any, route: str, direction: str, day: str) -> str:
        nonlocal repaired
        s = clean_station(name)
        if not s:
            return s
        other = "holiday" if day == "weekday" else "weekday" if day == "holiday" else ""
        if not other:
            return s
        own = station_sets.get((route, direction, day), set())
        opposite = station_sets.get((route, direction, other), set())
        if s in opposite:
            return s

        candidates: set[str] = set()
        # 曜日/方向見出しの一部が先頭・末尾へ付着した場合だけ候補化する。
        for n in (1, 2, 3):
            if len(s) <= n + 1:
                continue
            prefix, rest = s[:n], s[n:]
            if all(ch in marker_chars for ch in prefix) and rest in opposite:
                candidates.add(rest)
            suffix, rest2 = s[-n:], s[:-n]
            if all(ch in marker_chars for ch in suffix) and rest2 in opposite:
                candidates.add(rest2)

        if len(candidates) == 1:
            fixed = next(iter(candidates))
            # 自カレンダーにも元文字列が複数回正常駅として現れるなら変更しない。
            if s not in own or fixed in opposite:
                repaired += 1
                return fixed
        return s
    ...
    for t in trains:
        ...
        day = str(t.get("dayType", ""))
        for stop in t.get("stops", []):
            stop["station"] = repair(stop.get("station", ""), route, direction, day)
        if t.get("destination"):
            t["destination"] = repair(t.get("destination", ""), route, direction, day)
        stops = t.get("stops", [])
        if stops:
            t["origin"] = stops[0].get("station", "")

    print(f"Calendar station-name repairs: {repaired}")
    return trains
...
def main() -> int:
    ...
    # 平日/土休日PDFを相互照合して曜日見出し等の駅名混入を補正する。
    ...
    # 直通結合によって別セグメントから同一の完成列車が生成される場合がある。
    # Actions の重複判定より前に、完成列車単位でもう一度重複除去する。
    merged = final_dedupe(merged)
