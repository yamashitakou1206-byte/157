...
def coordinate_type_map(page) -> dict[str, str]:
    """表抽出で種別セルが崩れた時の保険。列車番号の直下近傍から種別を読む。"""
    try:
    ...
    for li, line in enumerate(lines):
        for w in line:
            num = normalize_train_number(_word_text(w))
            if not num or not TRAIN_RE.fullmatch(num):
                continue
            cx = (float(w.get("x0", 0)) + float(w.get("x1", 0))) / 2
            candidates: list[tuple[float, str]] = []
            # 種別は通常、列車番号行の数行下。同一X列を優先する。
            for lj in range(li, min(len(lines), li + 7)):
                near = []
                for ww in lines[lj]:
                    wcx = (float(ww.get("x0", 0)) + float(ww.get("x1", 0))) / 2
                    if abs(wcx - cx) <= 28:
                        near.append(ww)
                if not near:
                    continue
                # μ と S が別wordでも結合して判定
                joined = "".join(_word_text(x) for x in near)
                typ = normalize_type(joined)
                if typ:
                    ydist = abs(float(near[0].get("top", 0)) - float(w.get("top", 0)))
                    xdist = min(abs(((float(x.get("x0", 0)) + float(x.get("x1", 0))) / 2) - cx) for x in near)
                    candidates.append((ydist + xdist * 0.2, typ))
            if candidates:
                candidates.sort(key=lambda x: x[0])
                result.setdefault(num, candidates[0][1])
    return result
...
def parse_table(
    ...
    *,
    ...
    page_number: int,
    table_number: int,
    type_fallback: Optional[dict[str, str]] = None,
) -> list[dict[str, Any]]:
    if not raw_table:
        return []
    ...
    for block_no, header_i in enumerate(train_headers):
        ...
        for col, train_number in columns.items():
            ...
            if not train_type and type_fallback:
                train_type = type_fallback.get(train_number, "")
            if not train_type:
                continue
...
def parse_pdf(path: Path) -> list[dict[str, Any]]:
    ...
    with pdfplumber.open(path) as pdf:
        for page_no, page in enumerate(pdf.pages, start=1):
            type_fallback = coordinate_type_map(page)
