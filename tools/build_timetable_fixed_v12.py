...
def coordinate_station_lexicon(page) -> list[str]:
    """merge-14: 可視wordの幾何から駅名候補を復元する。

    平日/土休日などの欄外見出しを駅名へ連結しないことを最優先する。
    発/着/〃マーカーの直前wordを主候補とし、複数wordを連結する場合も
    カレンダー語・方向語・ヘッダ語を除外してから行う。
    """
    try:
        words = page.extract_words(x_tolerance=1.0, y_tolerance=1.6,
    ...
    for w in words:
        ...
        if x1 < 0 or x0 > float(page.width) or bot < 0 or top > float(page.height):
            continue
        vis.append(w)
    lines=_cluster_words_by_line(vis, tolerance=1.9)
    found=set()
    banned={'平','土','平日','土休日','休日','列車番号','列番','種別','行先','行き先',
            '記事','下り','上り','下','上','着','発','〃','駅名'}
    jp_re=re.compile(r'[^一-龥々ヶケぁ-んァ-ヶー]')
    for line in lines:
        if not line: continue
        line=sorted(line,key=lambda z:float(z.get('x0',0)))
        toks=[_dedupe_pdf_overlay_text(_word_text(z)).strip() for z in line]
        for marker,t in enumerate(toks):
            if not (t in {'発','着','〃'} or t.endswith(('発','着'))):
                continue
            # immediate predecessor is safest; table extraction often preserves it.
            candidates=[]
            if marker>0:
                candidates.append(toks[marker-1])
            # Join at most two adjacent Japanese tokens, never calendar/header tokens.
            if marker>1 and toks[marker-2] not in banned and toks[marker-1] not in banned:
                candidates.append(toks[marker-2]+toks[marker-1])
            # marker may be attached to station token itself.
            if t not in {'発','着','〃'}:
                candidates.append(re.sub(r'(?:発|着)$','',t))
            for raw in candidates:
                if raw in banned: continue
                raw=re.sub(r'^(?:平日|土休日|休日|下り|上り)+','',raw)
                cand=jp_re.sub('',raw)
                cand=clean_station(cand)
                if cand in banned: continue
                # A lone calendar prefix must never become part of a station name.
                if cand.startswith(('平日','土休日')):
                    continue
                if 2 <= len(cand) <= 14:
                    found.add(cand)
...
def repair_station_from_lexicon(value: Any, lexicon: Optional[list[str]]) -> str:
    ...
    if s in lexicon:
        return s
    # merge-14: 曜日見出し1字が駅名へ混入した場合。
    # suffix が同じページの座標辞書に実在する時だけ除去する。
    if len(s) >= 3 and s[0] in {"平", "土"} and s[1:] in lexicon:
        return s[1:]
...
def main() -> int:
    ...
    print("=" * 72)
    print("Meitetsu timetable parser v12 / merge-14-coordinate-stations")
    ...
    payload = {
        "version": 12,
        "build": "12-coordinate-guard-merge-14",
